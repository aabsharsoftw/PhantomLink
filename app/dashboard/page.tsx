...
function Spark({ data, color = 'var(--accent)', height = 36 }: { data: number[]; color?: string; height?: number }) {
...
function Kpi({ label, value, delta, deltaUp, sparkData, color, icon: Ico }: {
  ...
  sparkData: number[]; color: string; icon: (p: { size: number }) => ReactElement
}) {
  return (
    <div className="kpi">
...
function ChartCard({ title, children, subtitle, action }: { title: string; children: ReactNode; subtitle?: string; action?: ReactNode }) {
  return (
    <div className="card">
...
function ChannelBars() {
  const days = Array.from({ length: 14 }, (_, i) => ({
    d: i + 1,
    email: 200 + Math.round(Math.sin(i * 0.7) * 60 + 120),
    sms: 100 + Math.round(Math.cos(i * 0.5) * 30 + 70),
    wa: 150 + Math.round(Math.sin(i * 0.4 + 1) * 40 + 90),
  }))
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className="col" style={{ flex: 1, gap: 1, alignItems: 'stretch' }}>
              ...
              <div style={{ height: `${eh}px`, background: '#6366F1' }} />
            </div>
          )
        })}
      </div>
      ...
      <div className="page">
        ...
        <div className="dash-grid" style={{ marginTop: 16 }}>
          <ChartCard
            ...
            subtitle="New contacts per day · 30 days"
          ...
          <ChartCard
            ...
            subtitle="By channel · last 14 days"
            action={<button className="btn sm ghost">All channels <I.chevD size={11} /></button>}
          >
            <ChannelBars />
          </ChartCard>

          <ChartCard title="Recent activity" subtitle="Live feed across modules" action={<button className="btn sm ghost">View all</button>}>
            <ActivityFeed />
          </ChartCard>
          ...
          <ChartCard title="Campaign performance" subtitle="Last 30 days · all channels">
            <table className="table compact">
              <thead>
                <tr><th>Campaign</th><th>Channel</th><th>Sent</th><th>Open</th><th>Click</th><th>Reply</th></tr>
              </thead>
              <tbody>
