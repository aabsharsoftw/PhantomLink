...
const sites = [
  { name: 'Acme — Pricing Funnel',    type: 'funnel',  status: 'live',   pages: 5,  visits: '12.4K', conv: '8.4%',  rev: '₹4.2L', updated: '2h ago',  color: '#6366F1' },
  { name: 'Spring Promo Landing',      type: 'landing', status: 'live',   pages: 1,  visits: '4.8K',  conv: '14.2%', rev: '₹1.8L', updated: '1d ago',  color: '#22C55E' },
  { name: 'Webinar Registration',      type: 'funnel',  status: 'live',   pages: 3,  visits: '2.1K',  conv: '42.0%', rev: '—',     updated: '3d ago',  color: '#F59E0B' },
  { name: 'Acme HQ — Marketing Site',  type: 'website', status: 'live',   pages: 14, visits: '58K',   conv: '2.1%',  rev: '—',     updated: '5d ago',  color: '#A78BFA' },
  { name: 'Cart Recovery — WA Flow',   type: 'funnel',  status: 'paused', pages: 2,  visits: '820',   conv: '22.4%', rev: '₹62K',  updated: '2w ago',  color: '#EC4899' },
  { name: 'May Launch Teaser',         type: 'landing', status: 'draft',  pages: 1,  visits: '—',     conv: '—',     rev: '—',     updated: 'just now', color: '#14B8A6' },
]

function FakeSitePreview({ color, kind }: { color: string; kind: string }) {
...
function StatusPill({ status }: { status: string }) {
  if (status === 'live') return <span className="pill green"><span className="dot pulse" style={{ background: '#22C55E' }} /> Live</span>
  if (status === 'paused') return <span className="pill amber"><I.pause size={9} /> Paused</span>
  return <span className="pill"><I.pencil size={9} /> Draft</span>
}
...
export default function SitesPage() {
  const typeIconMap: Record<string, (p: { size: number; className?: string }) => ReactElement> = { funnel: I.funnel, landing: I.swatchSm, website: I.globe2 }
  return (
    <>
      <Topbar
        title="Sites & Funnels"
        subtitle={`${sites.length} sites · 4 live`}
        actions={<>
      ...
      <div className="page">
        ...
        <div className="sites-grid">
              ...
              <div key={i} className="card site-card">
                ...
                <div className="card-body">
                  ...
                  <div className="row gap-3" style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border-soft)', fontSize: 11 }}>
                    ...
                    <div className="col"><span className="muted-2">Conv</span><span style={{ fontWeight: 600 }}>{s.conv}</span></div>
          ...
          <div className="card site-card site-card-empty" style={{ minHeight: 280, border: '1px dashed var(--border)' }}>
