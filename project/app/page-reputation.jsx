// Reputation — review feed with sentiment + replies
function ReputationPage() {
  const stars = (n) => (
    <span className="row" style={{gap:1}}>
      {[1,2,3,4,5].map(i => <I.star key={i} size={11} fill={i<=n?'#F59E0B':'transparent'} stroke={1.5} style={{color:'#F59E0B'}}/>)}
    </span>
  );
  const reviews = [
                ...
                return (
                  <div key={r.n} className="row gap-2" style={{fontSize:11}}>
                    ...
                    <div style={{flex:1, height:5, background:'var(--surface-2)', borderRadius:3, overflow:'hidden'}}>
                      <div style={{width:`${(r.c/max)*100}%`, height:'100%', background:r.n>=4?'#22C55E':r.n>=3?'#F59E0B':'#EF4444'}}/>
                    </div>
          ...
          <div className="card flex-1" style={{padding:'18px 20px'}}>
            ...
            <div className="row gap-1" style={{flexWrap:'wrap'}}>
              {[['onboarding',42,'pos'],['support',28,'mix'],['automation',24,'pos'],['pricing',18,'neg'],['reporting',12,'mix']].map(([t,c,s]) => (
                <span key={t} className={`pill ${s==='pos'?'green':s==='neg'?'red':''}`}>{t} · {c}</span>
              ))}
            </div>
          </div>
        </div>
...
window.ReputationPage = ReputationPage;
