      ...
      <div className="page" style={{padding:0, display:'grid', gridTemplateColumns:'240px 1fr 280px', overflow:'hidden', height:'100%'}}>
              ...
              {edges.map((e,i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    {e.label && (
                      <g>
                        <rect x={(a.x+NODE_W + b.x)/2 - 16} y={(a.y+NODE_H/2 + b.y+NODE_H/2)/2 - 9} width="32" height="18" rx="9" fill="var(--surface)" stroke={stroke}/>
                        <text x={(a.x+NODE_W + b.x)/2} y={(a.y+NODE_H/2 + b.y+NODE_H/2)/2 + 3} textAnchor="middle" fontSize="10" fill={stroke} fontWeight="600">{e.label}</text>
                      </g>
                    )}
                  </g>
                );
        ...
        <aside style={{borderLeft:'1px solid var(--border)', overflowY:'auto'}}>
          <div style={{padding:'14px 16px', borderBottom:'1px solid var(--border)'}}>
            <div className="row gap-2"><div style={{width:24,height:24,borderRadius:6,background:'color-mix(in srgb, #22C55E 18%, transparent)',color:'#22C55E',display:'flex',alignItems:'center',justifyContent:'center'}}><I.wa size={13}/></div><span style={{fontWeight:600, fontSize:13}}>Send WhatsApp</span></div>
