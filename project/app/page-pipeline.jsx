...
function PipelinePage() {
  ...
  const totals = stages.map(s => ({ id:s.id, count:deals[s.id]?.length||0, val:(deals[s.id]||[]).reduce((a,d)=>a+parseInt(d.v.replace(/[^\d]/g,''))||0, 0) }));
  const tagColor = { 'enterprise':'violet','trial':'teal','agency':'amber','expansion':'blue','hot-lead':'red','mumbai':'blue','demo-booked':'green','intl':'pink','won':'green' };
  const prioColor = { high:'#EF4444', med:'#F59E0B', low:'#6C7187' };
