// Calendars / Appointments — GHL-style booking calendar
function CalendarsPage() {
  ...
  return (
    <>
      <Topbar
        title="Calendars" subtitle="Apr 28 – May 2 · 14 appointments"
        actions={<>
      ...
      <div className="page" style={{padding:0, display:'grid', gridTemplateColumns:'240px 1fr 280px', overflow:'hidden', height:'100%'}}>
        <aside style={{borderRight:'1px solid var(--border)', padding:'14px 12px', overflow:'auto'}}>
          ...
          <div className="col gap-2">
            {[{n:'Riya Acharya', s:'available', c:'#22C55E'},{n:'Aman Doshi', s:'in a call', c:'#EF4444'},{n:'Nikhil K.', s:'OOO Apr 30', c:'#F59E0B'}].map(p => (
        ...
        <div className="cal-grid">
          <div className="cal-head">
            ...
            {days.map((d,i) => (
              <div key={d} className={`cal-head-cell ${i===2?'today':''}`}>
          ...
          <div className="cal-body">
            ...
            {days.map((_, di) => (
              <div key={di} className={`cal-day-col ${di===2?'today':''}`}>
                {hours.map((h, hi) => <div key={hi} className="cal-slot"/>)}
        ...
        <aside style={{borderLeft:'1px solid var(--border)', padding:'14px 14px', overflow:'auto'}}>
          ...
          <div className="col gap-2">
            {[{n:'30-min discovery', s:'14 booked · 7 day window', c:'#6366F1'},
              ...
              {n:'Onboarding kickoff', s:'9 booked · 60 min slot', c:'#F59E0B'}].map(b => (
          ...
          <div className="col gap-2">
            {[{t:'Today 11:00', n:'Sana Iqbal · Discovery', who:'Riya', c:'#6366F1'},
              ...
              {t:'Tue 10:00',   n:'Faraz Khan · Demo', who:'Aman', c:'#22C55E'}].map((u,i) => (
...
function MiniCalendar() {
  ...
  const days = Array.from({length:35}, (_,i) => i - 2); // Apr starts Tue, padding
  return (
    <div className="card" style={{padding:10}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap:2}}>
        {dayNames.map(d => <div key={d} style={{fontSize:9, color:'var(--text-3)', textAlign:'center', padding:'2px'}}>{d}</div>)}
        {days.map((d,i) => {
